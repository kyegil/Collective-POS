...
function request($data = "") {
	...
	switch ($data) {
		case "productCombo":
			...
			$orderfields .= "IF(({$tp}products.name LIKE '%" . implode("%' OR {$tp}products.productCode LIKE '%", $searchset) . "%'), 0, 1),\n";
			$orderfields .= "IF(({$tp}products.brand LIKE '%" . implode("%' OR {$tp}products.productCode LIKE '%", $searchset) . "%'), 0, 1)";

			$result = $this->mysqli->arrayData(array(
				'source' => "{$tp}products LEFT JOIN {$tp}suppliers ON {$tp}products.supplier = {$tp}suppliers.id LEFT JOIN {$tp}product_attributes ON {$tp}products.id = {$tp}product_attributes.product LEFT JOIN {$tp}product_categories ON {$tp}products.id = {$tp}product_categories.product" ,
				'fields' => "{$tp}products.id, {$tp}products.name, productCode, price, {$tp}products.description, {$tp}products.unit, {$tp}products.inStock",
				'where' => $where,
